...
int get_option(int type, const char *msg)
{
	...
	case NUM:
		...
		while (getchar() != '\n')
			; // clears buffer
		if (num >= 0 && num <= 6)
		{ // checks if input is between valid range
			return num;
		}
	case CHAR:
		scanf("%c", &character);
...
Status add_contacts(AddressBook *address_book)
{
	...
	do
	{
		...
		printf("\n0. Back\n");
		printf("1. Name: [%s]\n",
			   strlen(address_book->list[address_book->count].name[0]) > 0 ? address_book->list[address_book->count].name[0] : "");
		printf("2. Phone Number(s):\n");
		for (int i = 0; i < PHONE_NUMBER_COUNT; i++)
		{
			printf("   %d. [%s]\n", i + 1,
				   strlen(address_book->list[address_book->count].phone_numbers[i]) > 0 ? address_book->list[address_book->count].phone_numbers[i] : "");
		}
		printf("3. Email(s):\n");
		for (int i = 0; i < EMAIL_ID_COUNT; i++)
		{
			printf("   %d. [%s]\n", i + 1,
				   strlen(address_book->list[address_book->count].email_addresses[i]) > 0 ? address_book->list[address_book->count].email_addresses[i] : "");
		}
		printf("4. Serial Number: [%d]\n",
			   address_book->list[address_book->count].si_no > 0 ? address_book->list[address_book->count].si_no : 0);
